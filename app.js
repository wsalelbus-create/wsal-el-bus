...
const mapView = document.querySelector('.map-view-container');

let startY = 0;
let currentY = 0;
let isDragging = false;

if (panelHandle && arrivalsPanel && mapView) {
    // Touch start
    panelHandle.addEventListener('touchstart', (e) => {
        startY = e.touches[0].clientY;
        isDragging = true;
        arrivalsPanel.style.transition = 'none'; // Disable transition during drag
    ...
    document.addEventListener('touchmove', (e) => {
        ...
        if (deltaY > 0) {
            const newPanelHeight = Math.max(20, Math.min(40, 40 - (deltaY / window.innerHeight * 100)));
            arrivalsPanel.style.height = `${newPanelHeight}vh`;
            mapView.style.height = `${100 - newPanelHeight}vh`;
        }
    }, { passive: true });
    ...
    document.addEventListener('touchend', () => {
        ...
        arrivalsPanel.style.transition = 'height 0.3s ease';
        mapView.style.transition = 'height 0.3s ease';

        const deltaY = currentY - startY;

        // Snap to collapsed (20vh) or default (40vh) based on drag distance
        if (deltaY > 100) {
            // Dragged down significantly - collapse panel, expand map
            arrivalsPanel.classList.add('collapsed');
            arrivalsPanel.style.height = '20vh';
            mapView.classList.add('expanded');
            mapView.style.height = '80vh';
        } else {
            // Snap back to default
            arrivalsPanel.classList.remove('collapsed');
            arrivalsPanel.style.height = '40vh';
            mapView.classList.remove('expanded');
            mapView.style.height = '60vh';
        }
    ...
    // Double tap on handle to toggle
    let lastTap = 0;
    panelHandle.addEventListener('touchend', (e) => {
        ...
        if (tapLength < 300 && tapLength > 0) {
            ...
            if (arrivalsPanel.classList.contains('collapsed')) {
                // Expand panel
                arrivalsPanel.classList.remove('collapsed');
                arrivalsPanel.style.height = '40vh';
                mapView.classList.remove('expanded');
