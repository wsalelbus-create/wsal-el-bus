...
// Panel Drag Functionality - Citymapper style
...
if (panelHandle && arrivalsPanel && mapContainer) {
    let startY = 0;
    let startHeight = 0;
    ...
    panelHandle.addEventListener('touchstart', (e) => {
        ...
        startHeight = arrivalsPanel.offsetHeight;
    ...
    document.addEventListener('touchmove', (e) => {
        ...
        // Update panel height
        const heightPercent = (constrainedHeight / viewportHeight) * 100;
        arrivalsPanel.style.height = `${heightPercent}%`;

        // Update map height (inverse)
        const mapPercent = 100 - heightPercent;
    ...
    document.addEventListener('touchend', () => {
        ...
        mapContainer.style.transition = 'height 0.3s ease';

        // Snap to nearest state
        const currentHeight = arrivalsPanel.offsetHeight;
        const viewportHeight = window.innerHeight;
        const heightPercent = (currentHeight / viewportHeight) * 100;

        if (heightPercent < 30) {
            // Snap to collapsed (20% panel, 80% map)
            arrivalsPanel.style.height = '20%';
            mapContainer.style.height = '80%';
            arrivalsPanel.classList.add('collapsed');
            mapContainer.classList.add('expanded');
        } else {
            // Snap to default (40% panel, 60% map)
            arrivalsPanel.style.height = '40%';
